...
const CurlVisualizer = ({ curlCommand }) => {
  ...
  return (
    ...
    <Card sx={{ p: 1, width: '100%', backgroundColor: '#f5f5f5',height: '15%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <p style={{ fontSize: '16px' }}>Request</p>
            <Tooltip title="Re-run">
                <IconButton onClick={() => executeRequest(editedCurl)} sx={{ '&:hover': { backgroundColor: '#e0e0e0' } }}>
                ...
                <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        {history.find(e => e.id === activeEntry)?.success ? (
                            <CheckCircle color="success" style={{ width: '15px', height: '15px', color: '#4caf50' }} /> 
                        ) : (
                            <Error color="error" style={{ width: '15px', height: '15px', color: '#f44336' }} />
                        )}
                        <Typography variant="h6" style={{ fontSize: '16px' }}>
                            Response - {history.find(e => e.id === activeEntry)?.response.status}
                        </Typography>
                        <IconButton
                            sx={{ marginLeft: 'auto', '&:hover': { backgroundColor: '#e0e0e0' } }}
                            onClick={() => navigator.clipboard.writeText(responseData)}
                        >
                            <ContentCopy style={{ width: '15px', height: '15px' }} />
                        </IconButton>
                    </div>
                    <div style={{ overflowY: 'auto', maxHeight: '40vh', overflow: 'scroll' }}>
                        <SyntaxHighlighter
                            sx={{ overflowY: 'auto' }}
                            language="json"
                            style={atomDark}
    ...
    <Grid style={{ width: '98%', height: '30%' }}>
      <Card sx={{ paddingBottom: '1rem', backgroundColor: '#f0f0f0', width: '100%', height: '100%', maxHeight: '100%', padding: '0.6rem',
          border: '1px solid #e0e0e0'}}>
          <Typography variant="h6" gutterBottom color="text.secondary" style={{ fontSize: '16px' }}>History</Typography>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, overflow: 'auto', maxHeight: 'calc(100% - 40px)', justifyContent: 'flex-start', alignItems: 'center' }}>
              {history.map((entry) => (
                  <motion.div
                      key={entry.id}
                      initial={{ opacity: 0 }}
                  ...
                  >
                      <Card
                          sx={{
                              ...
                              p: 1,
                              cursor: 'pointer',
                              backgroundColor: activeEntry === entry.id ? '#e8f5e9' : 'background.paper',
                      ...
                      >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                              {entry.success ? (
                                  <CheckCircle color="success" fontSize="small" style={{ color: '#4caf50' }} />
                              ) : (
                                  <Error color="error" fontSize="small" style={{ color: '#f44336' }} />
                              )}
